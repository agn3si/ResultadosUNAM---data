...
def fit_bb(hist: np.ndarray) -> dict[str, float]:
    ...
    if not fit.success:
        print("Fit with L-BFGS-B failed, fallback to Powell.")
        fallback = minimize(
            objective,
            initial,
            method="Powell",
...
def fit_mixture(hist: np.ndarray, usual_alpha: float, usual_beta: float) -> dict[str, float]:
    """Fit the high-score component and its fraction, NOT holding usual BB fixed."""
    scores = np.arange(N_QUESTIONS + 1)
    ...
    # theta = log(a_atypical), log(b_atypical),
    # log(a_boosted), log(b_boosted), 
    # log(frac_atypical)
    initial = np.array([
        np.log(usual_alpha),
        np.log(usual_beta),
        np.log(max(sample_mean * 8, 0.1)),
        np.log(max((1 - sample_mean) * 8, 0.1)),
        0.5
    ])
    ...
    def objective(theta: np.ndarray) -> float:
        a_low = np.exp(theta[0])
        b_low =  np.exp(theta[1])
        a_boosted = np.exp(theta[2])
        b_boosted = np.exp(theta[3])
        fraction = theta[4]
        low_population = bb_logpmf(scores, a_low, b_low)
        boosted_population = bb_logpmf(scores, a_boosted, b_boosted)
        # aa, ab, _, _, fraction = unpack(theta)
        if np.isclose(fraction,0):
            components = np.vstack(
                (low_population, np.full_like(low_population,-np.inf))
            )
        elif np.isclose(fraction,1):
            components = np.vstack(
                (np.full_like(low_population,-np.inf), boosted_population)
            )
        else:
            components = np.vstack(
                (np.log1p(-fraction) + low_population, np.log(fraction) + boosted_population)
            )
        return float(-np.dot(hist, logsumexp(components, axis=0)))

    best = None
    for log_alpha in (-2.0, 0.0, 2.0):
        for log_beta in (-2.0, 0.0, 2.0):
            start = initial.copy()
            start[0] += log_alpha
            start[1] += log_beta
            start[2] += log_alpha
            ...
            candidate = minimize(
                ...
                start,
                method="L-BFGS-B",
                bounds=[(0, 12), (0, 12), (0, 12), (0, 12), (0, 1)],
            )
    ...
    log_la, log_lb, log_aa, log_ab, fraction = best.x
    la = np.exp(log_la)
    lb = np.exp(log_lb)
    aa = np.exp(log_aa)
    ab = np.exp(log_ab)
    ...
    return {
        ...
        "low_mean_score": float(N_QUESTIONS * lmean),
        "low_score_sd": bb_score_sd(la, lb),
