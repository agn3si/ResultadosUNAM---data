...
def fit_mixture(hist: np.ndarray, usual_alpha: float, usual_beta: float) -> dict[str, float]:
    ...
    # theta = log(a_atypical), log(b_atypical), log(frac_atypical)
    initial = np.array([
    ...
    def objective(theta: np.ndarray) -> float:
        ...
        if np.isclose(fraction,0):
            components = np.vstack(
                (log_usual, np.full_like(log_usual,-np.inf))
            )
        elif np.isclose(fraction,1):
            components = np.vstack(
                (np.full_like(log_usual,-np.inf), bb_logpmf(scores, aa, ab))
            )
        else:
            components = np.vstack(
